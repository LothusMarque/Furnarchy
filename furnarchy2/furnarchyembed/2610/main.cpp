...
bool patch_client_image( )
{
   ...
   size_t max_size = 0;
   char* strbuf = image_locate_header_surplus( g_client_image, &max_size );
   char* p = strbuf;
   ...
   *strrchr( p, '\\' ) = '\0';
   ...
   char* loader = image_locate_code_surplus( g_client_image, &max_size );
   ...
   #define VIRT( p ) ((size_t) (p) - (size_t) g_client_image + (size_t) GetModuleHandle( 0 ))
   ...
   image_set_entry_point( g_client_image, 
                          (void*) ((size_t)loader - (size_t)g_client_image) );
