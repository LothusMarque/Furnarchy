...
bool   g_rawmode     = true;
int    g_rawscript   = 0; //Script handle!
bool   g_timestamp   = false;
bool   g_timestampsecs = false;
...
/*
   ...
   if (arg == "rawchat")
   {
      if (args.size( ) == 2)
      {
         ...
         {
			ArgList arglist;
         arglist.push_back( "rawchat" );
     // return 
		  scripts::script_run( arglist, SCRIPT_RUN_UNIQUE );
            g_rawmode = true;
         ...
         {
         ArgList arglist;
		 arglist.push_back( "sckill" );
         arglist.push_back( "rawchat" );
		 scripts::cmd_sckill(arglist);
            //g_rawmode = false;
	   ...
	   if (arg == "timestamp")
   {
	...
	if (arg == "timestampsecs")
    {
      if (args.size( ) == 2)
      {
         ...
         if (arg == "on")
         {
            g_timestampsecs = true;
            core::system_speak( "Seconds in timestamping enabled." );
            return true;
         }
         else if (arg == "off")
         {
            g_timestampsecs = false;
            core::system_speak( "Seconds in timestamping disabled." );
            return true;
         }
      }
      core::system_speak( "Usage: @timestampsecs <'on'|'off'>" );
      return true;
    }
   else if (arg == "setws")
   {
 ...
 /*  if (memlocs::valid( ))
   {
      ...
      bool unk1 = 0, unk2 = 0;
      void *unk1_addr = &unk1, *unk2_addr = &unk2;
      int len = strlen( line );
      int rval;
      __asm
      {
...
int CDECL patches::chatbox_push( char* line, void* unk1, void* unk2 )
{
   ...
   if (strcmp( line, "Retrying..." ) && !cancel)
   {
      ...
      if (g_timestamp && line[ 0 ] != '#')
      {
		 ...
		 if (g_timestampsecs) { ss << ":" << std::setw( 2 ) << std::setfill( '0' ) << st.wSecond; }
         ss << ")</font> "
            << line;
