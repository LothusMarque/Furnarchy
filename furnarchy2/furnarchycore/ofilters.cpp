...
struct FILTER
{
   ...
   FILTER( ) : options( 0 ), hide_flags( 0 ), reop( 0 ) { }
   ~FILTER( ) { delete reop; }
};
...
list<FILTER*> g_filters;
 ...
 bool cmd_ofadd( const ArgList& arg_list );
 bool cmd_ofrm( const ArgList& arg_list );
 void cmd_ofls( const ArgList& arg_list );
 void cmd_ofon( const ArgList& arg_list );
 void cmd_ofoff( const ArgList& arg_list );
 bool cmd_ofmv( const ArgList& arg_list );
 void cmd_ofuse( const ArgList& arg_list );
 bool cmd_ofclr( const ArgList& arg_list );
 bool load_filters( );
 bool write_filters( );
 void clear_filters( );
...
void cmd_ofon( const ArgList& arg_list )
{
   ...
   g_enabled = true;
   core::system_speak( "ofilters enabled." );
}
...
void cmd_ofoff( const ArgList& arg_list )
{
   ...
   g_enabled = false;
   core::system_speak( "ofilters disabled." );
}
...
void cmd_ofuse( const ArgList& arg_list )
{
   ...
   core::system_speak( "Using \"" + g_use + "\" ofilters." );
}
...
bool on_command( const ArgList& arg_list )
{
   ...
   if (arg == "ofadd")
   {
      if (cmd_ofadd( arg_list ))
         write_filters( );
      return true;
   }
   if (arg == "ofrm")
   {
      if (cmd_ofrm( arg_list ))
         write_filters( );
      return true;
   }
   if (arg == "ofmv")
   {
      if (cmd_ofmv( arg_list ))
         write_filters( );
      return true;
   }
   if (arg == "ofls")
        ...
        !(hide_flags & HIDE_FILTERS) && i != g_filters.end( ); ++i)
   {
      ...
      if ((*i)->options & OPT_CHAT)
      {
         ...
         case '-':
            ...
            break;
         case 'w':
            if (line->c_str( )[ 1 ] == 'h' && line->c_str( )[ 2 ] == ' ')
            {
               /* Need to skip ahead to the message body. */
               const char* p = line->c_str( ) + 3;
               for(; *p && *p != ' '; ++p) ;
               if (*p == ' ')
               {
                  tmp.assign( *line, 0, (size_t) p - (size_t) line->c_str( ) + 1 );
                  match = (*i)->reop->execute( line->c_str( ) + tmp.length( ), 
                                               line->length( ) - tmp.length( ), &tmp );
               }
            }
            break;
         default:
...
bool load_filters( )
{
   ...
   if (!fs.fail( ))
   {
      ...
      {
         getline( fs, line );
         make_arg_list( "ofadd " + line, &arg_list );
         if (arg_list.size( ) > 1)
            cmd_ofadd( arg_list );
      }
