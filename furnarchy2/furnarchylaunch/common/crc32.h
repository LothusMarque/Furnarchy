...
inline unsigned int crc32_bytes( const void* p, std::size_t size )
{
   unsigned int crc = 0xffffffff;
   const void* const e = (const char*) p + size;

   while (p != e)
      crc = ( crc >> 8 ) ^ crc32_lut[ *(((const unsigned char*&)p)++) ^ (crc & 0xff) ];
   return ~crc;
}

inline unsigned int crc32_stream( std::istream& is )
{
   unsigned int crc = 0xffffffff;
   unsigned char buf[ 256 ];
   const unsigned char *p, *e;

   while (is.read( (char*) buf, 256 ).gcount( ))
   {
      p = buf;
      e = p + is.gcount( );

      while (p != e)
         crc = ( crc >> 8 ) ^ crc32_lut[ *(p++) ^ (crc & 0xff) ];
   }
