...
string format( const string& _pattern, const list<FIELD>& _fields )
{
   const char* pattern = _pattern.c_str( );
   list<FIELD>::const_iterator fields = _fields.begin( );
   string r;
   ...
   while (*pattern)
   {
      ...
      if (pattern[ 0 ] == '%')
      {
         ...
         case 'c': // One or more characters.
            {
               if (fields != _fields.end( ))
               {
                  r.append( fields->value_str );
                  ++fields;
               }
               pattern  += 1;
               break;
         ...
         case 's': // Byte-string.
            {
               if (fields != _fields.end( ))
               {
                  r.push_back( yiffurc::itof1_220( fields->value_str.length( ) ) );
                  r.append( fields->value_str );
                  ++fields;
               }
               pattern += 1;
         ...
         case 'n': // Base-95 digit.
            {
               if (fields != _fields.end( ))
               {
                  ...
                  {
                     unsigned n = fields->value_int;
                     std::string str( field_len, ' ' );
                     unsigned modulo = 1;
         ...
         case 'N': // Base-220 digit.
            {
               if (fields != _fields.end( ))
               {
...

                  {
                     unsigned n = fields->value_int;
                     std::string str( field_len, '#' );
                     for (unsigned i = 0; n && i < field_len; ++i)
                     {
...
bool scan( const string& _pattern, const string& subj, list<FIELD>& fields )
{
   const char* pattern = _pattern.c_str( );
   int subj_pos = 0, subj_len = subj.length( );
   ...
   while (subj_pos < subj_len && pattern[ 0 ])
   {
      ...
      if (pattern[ 0 ] == '%')
      {
         ...
         case 'K': // New-style color code.
            {
               field_len = yiffurc::get_color_len( subj.c_str( ) + subj_pos, subj_len - subj_pos );
