   ...
   while (m_off < m_len)
   {
      ...
      if (m_buf[ m_off ] == '<')
      {
         ...
         if (n && m_tag->type != Tag::TAG_BAD)
         {
            ...
            else if (m_tag->isName( "img" ))
            {
               if (m_tag->type == Tag::TAG_EMPTY)
               {
                  m_last_token.type  = TOK_IMG;
                  m_last_token.off   = m_off;
                  m_last_token.len   = n;
                  m_last_token.href  = m_href.empty( ) ? "" : m_href.top( );
                  m_last_token.color = m_color.empty( ) ? "" : m_color.top( );
                  ...
                  if (m_tag->isAttr( "src" ))
                     m_last_token.src = m_tag->getAttrVal( "src" );
                  else
                     m_last_token.src.clear( );

                  m_off += n;
                  break;
               }
            }
            else if (m_tag->isName( "font" ))
            {
               if (m_tag->type == Tag::TAG_START)
               {
                  if (m_tag->isAttr( "color" ))
                     m_color.push( m_tag->getAttrVal( "color" ) );
               }
               else if (m_tag->type == Tag::TAG_END)
               {
                  if (!m_color.empty( ))
            ...
            else if (m_tag->isName( "name" ))
            {
               if (m_tag->type == Tag::TAG_START)
               {
                  if (m_tag->isAttr( "shortname" ))
                     m_href.push( "name://" + m_tag->getAttrVal( "shortname" ) );
               }
               else if (m_tag->type == Tag::TAG_END)
               {
                  if (!m_href.empty( ))
